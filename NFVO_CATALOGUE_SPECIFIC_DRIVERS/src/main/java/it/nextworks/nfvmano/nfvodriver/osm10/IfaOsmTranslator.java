...
import it.nextworks.nfvmano.libs.descriptors.sol006.OsmNsSol6Package;
import it.nextworks.nfvmano.libs.descriptors.sol006.OsmNsdSol6Catalogue;
...
public class IfaOsmTranslator {
    ...
    // root directory where osm nsd package are stored
    ...
    private static NSDescriptor translateIfaToOsmNsd(Nsd nsd, NsDf nsDf, HashMap<String, Boolean> useTemplateVNFDs) {
        ...
        for (NsVirtualLinkDesc nsVirtualLinkDesc : nsd.getVirtualLinkDesc()) {
            ...
            if(isVLMgmt)
                vld.setVimNetworkName("VM_MGMT");
            else if(nsVirtualLinkDesc.getVirtualLinkDescId().equals("user_net"))
                vld.setVimNetworkName("user_net");
            else if (nsVirtualLinkDesc.getVirtualLinkDescId().equals("origin_net"))
                vld.setVimNetworkName("origin_net");
            else if(nsVirtualLinkDesc.getVirtualLinkDescId().equals("cache_net"))
                vld.setVimNetworkName("cache_net");
            //if vim network name not specified, openstack will create it
    ...
    private static VNFDescriptor translateIfaToOsmVnfd(Vnfd vnfd, VnfDf vnfDf) throws FailedOperationException {
        ...
        /*
        Not relevant in vnf with one single vdu
        ...
        for(VnfVirtualLinkDesc vnfVirtualLinkDesc : vnfd.getIntVirtualLinkDesc()){
            ...
            vld.setType("ELAN"); // or maybe ELINE?
            //vld.setRootBandwidth();  How to set this? aggregate bandwidth
            //vld.setLeafBandwidth();  How to set this? bandwidth of branches
    ...
    //TODO
    public static File createPackageForPnfd(Pnfd pnfd) {

        VNFDescriptor vnfDescriptor = new VNFDescriptor();

        //set generic info
        vnfDescriptor.setId(pnfd.getPnfdId());
        vnfDescriptor.setName(pnfd.getPnfdId());
        vnfDescriptor.setShortName(pnfd.getPnfdId());
        vnfDescriptor.setVendor(pnfd.getProvider());
        vnfDescriptor.setVersion(pnfd.getVersion());
        List<it.nextworks.nfvmano.libs.osmr4PlusDataModel.vnfDescriptor.ConnectionPoint> connectionPoints = new ArrayList<>();
        PnfExtCpd mgmtConnectionPoint = null; //taking the management ExtCp
        for(PnfExtCpd pnfExtCpd : pnfd.getPnfExtCp()){
            it.nextworks.nfvmano.libs.osmr4PlusDataModel.vnfDescriptor.ConnectionPoint connectionPoint
                    = new it.nextworks.nfvmano.libs.osmr4PlusDataModel.vnfDescriptor.ConnectionPoint();
            connectionPoint.setId(pnfExtCpd.getCpdId());
            connectionPoint.setType("VPORT");
            connectionPoint.setName(pnfExtCpd.getCpdId());
            if(mgmtConnectionPoint == null && checkIfIsManagement(pnfExtCpd.getAddressData())) mgmtConnectionPoint = pnfExtCpd;
            connectionPoints.add(connectionPoint);
        }
        vnfDescriptor.setConnectionPoints(connectionPoints);
    ...
    /**
     * Create the Nsd zip package and return the File
     * @param nsd
     * @return String
     */
    public static File createPackageForNsd(it.nextworks.nfvmano.libs.descriptors.sol006.Nsd nsd) {

        File nsdFolder = makeNsFolder(nsd.getId());

        //making yaml file
        List<it.nextworks.nfvmano.libs.descriptors.sol006.Nsd> nsds = new ArrayList<>();
        nsds.add(nsd);
        OsmNsdSol6Catalogue catalogue = new OsmNsdSol6Catalogue(nsds);
        OsmNsSol6Package sol6Package = new OsmNsSol6Package(catalogue);
        makeYml(sol6Package,nsd.getId(),nsdFolder.getAbsolutePath());
    ...
    private static void makeYml(OsmNsSol6Package sol6Package, String nsdId, String nsdFolder) {
        ...
        try{
            List<String> strings = Arrays.asList(ymlMapper.writeValueAsString(sol6Package).split("\n"));
